...
**Доказательство**
	...
	$= \sum\limits_{i=1}^n \left(|x_i + y_i|\cdot |x_i| + |x_i + y_i|\cdot |y_i|\right) \le \|x\|_p \left( \sum\limits_{i=1}^n |x_i y_i|^{(p-1)q} \right) ^ \frac 1 q \le \|x\|_p \left( \sum\limits_{i=1}^n |x_i y_i|^{p} \right) ^ \frac 1 q$
