...
Нам нужно, чтобы сходились нормы $k$ и нормы $g$.
Для того, чтобы сходилась норма $g$, нужно чтобы $\int \limits_{ -1 }^{ 1 }\left| s \right|^{-\frac q 3}ds$ сходился. Он сходится тогда, когда $-\dfrac{q}{3} > -1$, т. е. $q < 3$
$\implies \dfrac{p}{p - 1} < 3 \implies p > \dfrac{3}{2}$

**Для $p = 1$**
	$L_{\infty}\left[ -1, 1 \right]$
