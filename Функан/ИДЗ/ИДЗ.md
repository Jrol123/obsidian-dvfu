...
Проверим $Ax$ на корректность.


Докажем, что $A$ - сжимающий оператор.
$\left\| A \right\| = \underset{ k \in \mathbb{N} }{ \sup } \sum \limits_{ j = 1 }^{ \infty }\left| a_{kj} \right| \leq \alpha < 1$, следовательно A - сжимающий оператор.
