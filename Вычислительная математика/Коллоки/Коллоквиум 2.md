...
Из $(3)$ следует: $\sum\limits_{i \ne j} b_{ij}^2 \le \left(1 - \dfrac{2}{n(n-1)}\right) \sum_{i \ne j} a_{ij}^2$.
ч. т. д.


Рассмотрим метод вращений в целом. 
На первом шаге находим индексы $l_1, m_1$ максимального по модулю наддиагонального элемента $A$, строим матрицу вращения $C_1 = C_{l_1 m_1}$ с определёнными выше опорными элементами, и вычисляем матрицу $A_1 = C_1^T AC_1$.
...
Произведение $U_k = C_1 \ldots C_k$ ортогональных матриц ортогонально, потому $A_k$ подобна $A$, следовательно их собственные значения совпадают. Покажем, что при $k \to \infty$ $A_k$ стре
