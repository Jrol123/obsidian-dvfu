...
Сумма размерностей $R_{n - i + 1}$ и $S_{i}$ равна $n + 1$, т. е. превышает размерность $n$ рассматриваемого пространства.
Следовательно, существует вектор $x_{0} \neq 0 \in R_{n - i + 1}, S_{i}$
...
#### Теорема
Пусть все собственные значения невырожденной матрицы $A$ вещественны и различны по модулю, и пусть все главные миноры матрицы $T^{-1}$ из равенства $(2)$ отличны от нуля. Тогда последовательность матриц $A_k$, определённых по правилу $(1)$, 
...
$\lambda_{1}(A) = \dfrac{\left( Ax^{(k)}, x^{(k)} \right)}{(x^{(k)}, x^{(k)})} = \dfrac{(x^{(k)})^{T}A^{(k)}}{(x^{(k)})^{T}x^{(k)}}$
### Доказательство
...
### Доказательство
Поскольку СЗ обратной матрицы обратны собственным значениям исходной матрицы, а собственные вектора совпадают с собственными векторами исходной матрицы, то последовательной $x^{(k)}$ будет сходиться к СВ исходной матрицы $A$, отвечающему её наименьшему значению