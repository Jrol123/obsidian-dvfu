...
$\left( x, x \right) = \sum \limits_{ j = 1 }^{ i }c_{j}^{2} \quad \left( Ax, x \right) = \sum \limits_{ j = 1 }^{ i }(A)$
...
$\sum\limits_{i \ne j} b_{ij}^2 - \sum\limits_{i \ne j} a_{ij}^2 = \sum\limits_{i=1}^n a_{ii}^2 - \sum\limits_{i=1}^n b_{ii}^2 a_{ll}^2 + a_{mm}^2 - b_{ll}^2 - b_{mm}^2$
