...
Через $\Lambda_k$ обозначим матрицу из диагональных элементов $A_k$ (все остальные элементы равны нулю), а через $\delta_k$ обозначим $\delta_k = A_k - \Lambda_k$.
Пусть собственные значения матриц $A_k$ и $\Lambda_k$ пронумерованы в порядке неубывания. Тогда согласно следствию 2 из вариационного принципа Куранта-Фишера имеют место оценки:
