...
Так как $-\dfrac{\pi}{2} \lt 2\phi \le \dfrac{\pi}{2}$, то $\cos 2\phi \le 0$.
Поскольку $\cos \psi \le 0$, то в формуле $(2)$ следует оставить только ***ЧЕТЫРЕ*** значения $k$. Таким образом, $\psi = 2\psi + 2\pi k, k = 0, 1, \ldots$
