...
$|\lambda_i(A_k) - \lambda_i(\Lambda_k)| \le ||\epsilon_k||_2 \cdot ||\epsilon_k||_E, \quad i = \overline{1,n}$
Но $\lambda_i(A_k) = \lambda_i(\Lambda_k), \lambda_i(\Lambda_k) = a_{ii}^{(k)}$. Следовательно, 
$|]lambda_i(A)|$
