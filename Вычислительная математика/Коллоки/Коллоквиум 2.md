...
$$\tan 2\pi = \dfrac{x}{y} = \dfrac{\frac{\operatorname{sign}(xy)|x|}{\sqrt{x^2 + y^2}}}{\frac{|y|}{\sqrt{x^2 + y^2}}} = \dfrac{\overline{s}}{\overline{c}} \tag{1}$$
где $\overline{s} = \dfrac{\operatorname{sign}(xy)|x|}{\sqrt{x^2 + y^2}}, \overline{c} = \frac{|y|}{\sqrt{x^2 + y^2}}$

Поскольку $\overline{c}^2 + \overline{s}^2 = 1$, то $\overline c$ и $\overline s$ также можно рассматривать как синус и косинус некоторого угла: $\overline{c} = \cos \phi, \overline s = \sin \phi$.

Из $(1)$ следует, что $\tan 2\phi = \tan$
