...
**LU - разложение**
	...
	*Если $i\ge j:$
		$l_{ij}=a_{ij}-\sum\limits_{k=1}^{j-1} l_{ik}\cdot u_{kj}$
	Если $i<j:$*
		$l_{ij}=\dfrac{a_{ij}-\sum\limits_{k=1}^{i-1} l_{ik}\cdot u_{kj} }{l_{jj}}$

**Необходимое и достаточное условие существования**
	Все угловые миноры матрицы $A$ невырожденые

**Замечание**
	После LU разложения можно легко вычислить определитель $|A|=\prod\limits_{i=1}^{n}l_{ii}$
