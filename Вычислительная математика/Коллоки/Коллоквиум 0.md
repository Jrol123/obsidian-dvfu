...
$$
\begin{matrix}
{k=a_{22}-a_{21}\cdot A_{i-1}^{-1}\cdot a_{12}} \\
\begin{cases}
{B_{i-1} = \dfrac{1}{k}\cdot A_{i-1}^{-1}\cdot \left( k\cdot E+a_{12}\cdot a_{21}\cdot A_{i-1}^{-1} \right)} \\
...
Таким образом,
$A_{i}^{-1} = \begin{pmatrix}A_{n - 1}^{-1} + \frac{1}{k}A_{n - 1}A^{-1}a_{12}a_{21}A_{n - 1}^{-1} & -\frac{1}{k}A_{n - 1}^{-1}a_{12} \\ -\frac{1}{k}a_{21}A^{}\end{pmatrix}$