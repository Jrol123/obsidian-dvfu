...
**$QR$ разложение**
	...
	*Из-за отображения, у нас меняются все элементы*
	...
	$a_{j}^{(k)} = a_{j}^{(k - 1)} - 2\dfrac{p^{(k)}\cdot a_{j}^{(k - 1)}}{\left( p^{(k)}, p^{(k)} \right)}p^{k}$
	$a_{ij}^{(k)} = a_{ij}^{(k - 1)} - 2 \dfrac{p_{i}^{(k)}}{\sum \limits_{ l=1 }^{ n }\left( p_{l}^{(k)} \right)^{2}}\sum \limits_{ l=k }^{ n }p_{l}^{(k)}a_{lj}^{(k - 1)} \quad j = k + 1, \ldots, n$
	%% $p_{1}^{(k)} = p_{2}^{(k)} = \ldots = p_{k - 1}^{(k)} = 0$ %%
