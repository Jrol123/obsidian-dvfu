...
**Метод последовательной верхней релаксации**
	...
	**Тогда** $r_{i} = t_{i} - a_{ii}\omega\dfrac{t_{i}}{a_{ii}} = t_{i}\left( 1 - \omega \right)$
	...
	При подстановке в расчётные формулы для $r_{i}$ получаем: $B = D + \omega A_{L} \quad \tau = \omega$
	**Скалярный вид:**
...
**Метод наискорейшего градиентного спуска**
	...
	**Условия:**
	...
	$\phi'(\Delta_{k}) = \left[ \left( Ay^{(k + 1)}, x^{(k + 1)} \right) - 2\left( f, x^{(k + 1)} \right) \right]' = 0$
	$\left( A \dfrac{dx^{(k + 1)}}{d\Delta_{k}}, x^{(k + 1)} \right) + \left( Ax^{(k + 1)}, \dfrac{dx^{(k + 1)}}{d\Delta_{k}} \right) - 2\left( f, \dfrac{dx^{(k + 1)}}{d\Delta_{k}} \right) = 2\left( Ax^{(k + 1)} - f, \dfrac{dx^{(k + 1)}}{d\Delta_{k}} \right) = 0$
	$\dfrac{dx^{(k + 1)}}{d\Delta_{k}} = -\left( Ax^{(k)} - f \right)$
	$-2\left( Ax^{(k + 1)} - f, Ax^{(k)} - f \right) = 0$
	$\left( Ax^{(k)} - f - \Delta_{k}A\left( Ax^{(k)} - f \right), Ax^{(k)} - f \right) = 0$
	Откуда $\Delta_{k} = \dfrac{\left( Ay^{(k)} - f, Ay^{(k)} - f \right)}{\left( A\left( Ay^{(k)} - f \right), Ay^{(k)} - f \right)}$
