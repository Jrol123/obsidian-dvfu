...
**Метод наискорейшего градиентного спуска**
	...
	Если $y \neq x^{*}$:
		$F(y) > F(x^{*})$
	$F = \sum \limits_{ i=1 }^{ n }\left( Ax_{i} \right)x_{i} - 2\sum \limits_{ i=1 }^{ n }f_{i}x_{i}$
	$\sum \limits_{ j=1 }^{ n }\sum \limits_{ i=1 }^{ n }a_{ij}x_{i}x_{j} - 2\sum \limits_{ i=1 }^{ n }f_{i}x_{i}$
	$\nabla F = 2\left( Ax - f \right)$
	Пусть $\phi(\Delta_{k}) = F(y^{(k + 1)})$
	Вычислим $\phi'(\Delta_{k})$
	$\phi(\Delta_{k}) = F(y^{(k)}) - 2\Delta_{k}\left( Ay^{(k)} - f, Ay^{(k)} - f \right) + \left[ A\left( Ay^{(k)} - f \right), Ay^{(k)} - f \right]\Delta_{k} - \left( Ay^{(k)} - f, Ay^{(k)} - f \right) = 0$
