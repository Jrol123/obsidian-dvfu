...
$u_{r}\left( a, \phi \right) = \sum \limits_{ k = 1 }^{ \infty }ka^{-k-1}\left( A_{k} \cos \left( k \phi \right) + B_{k} \sin \left( k\phi \right) \right) = 7 - \cos ^{5}\left( \phi \right)$
$\cos ^{5}\left( \phi \right) = \cos^{4}\left( \phi \right) \cdot \cos ^{1}\left( \phi \right) = \cos \left( \phi \right)\cdot \dfrac{3 + 4 \cos \left( 2\phi \right) + \cos \left( 4\phi \right)}{8}$
