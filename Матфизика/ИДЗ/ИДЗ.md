...
## Ответ
$u\left( x, t \right) = -\dfrac{2x^{3} + 3x^{2}}{6a^{2}} + \left( 1 + \dfrac{5}{6a^{2}} \right)x + 1 + \sum \limits_{ m = 0 }^{ \infty } \dfrac{6\cdot \left({-1}\right)^{m} - 2}{{\pi}^{3}\,{a}^{2}\,{m}^{3}} \sin \left( m\pi x \right) \cdot \exp \left( -a^{2}\left( m\pi \right)^{2} t \right)$
