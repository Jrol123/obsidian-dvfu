...
$u_{r}\left( r, \phi \right) = \sum \limits_{ k = 1 }^{ \infty } -kr^{-k-1}\left( B_{k}\sin \left( k\phi \right) + A_{k}\cos \left( k\phi \right) \right)$
$u_{r}\left( a, \phi \right) = \sum \limits_{ k = 1 }^{ \infty }ka^{-k-1}\left( A_{k} \right)$
