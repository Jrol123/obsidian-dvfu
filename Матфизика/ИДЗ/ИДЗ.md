...
$v\left( x, 0 \right) = \sum \limits_{ n = 0 }^{ \infty } \alpha _{n}\sin \left( n\pi x \right) \cdot 1 = \dfrac{2x^{3} + 3x^{2} - 5x}{6a}$
Домножим скалярно обе части на $\sin \left( m\pi x \right)$
$\int \limits_{ 0 }^{ 1 } \sin \left( n\pi x \right)\sin \left( m\pi x \right)dx = \begin{cases}  \end{cases}$
