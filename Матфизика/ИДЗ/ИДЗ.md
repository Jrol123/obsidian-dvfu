...
$u_{r}\left( a, \phi \right) = \sum \limits_{ k = 1 }^{ \infty }ka^{-k-1}\left( A_{k} \cos \left( k \phi \right) + B_{k} \sin \left( k\phi \right) \right) = 7 - \cos ^{5}\left( \phi \right) = 7 - \dfrac{10 cos(x) + 5 cos(3x) + cos(5x)}{16}$
**Проверка на разрешимость**
	$\int \limits_{ 0 }^{ 2\pi }7 - \cos ^{5}\left( \phi \right)d\phi = 14 \pi \neq 0 \implies$ задача неразрешима
	С $7-$ задача не решается, потому, пока не понятно как решить с ней, уберём её
...
## Ответ
$u\left( r, \phi \right) = A_{0} + \dfrac{10}{16}a^{2}\cos \left( \phi \right)$