...
$u_{r}\left( a, \phi \right) = \sum \limits_{ k = 1 }^{ \infty }ka^{-k-1}\left( A_{k} \cos \left( k \phi \right) + B_{k} \sin \left( k\phi \right) \right) = 7 - \cos ^{5}\left( \phi \right) = 7 - \cos \left( \phi \right)\cdot \dfrac{3 + 4 \cos \left( 2\phi \right) + \cos \left( 4\phi \right)}{8}$$B_{k} = 0  \quad \forall k > 0$
