...
$$
\dfrac{1}{\tau^{2}} \left( u_{i}^{j + 1}-2u_{i}^{j} + u_{i}^{j - 1} \right)=\dfrac{a^2}{2}\left(   \dfrac{a^{2}}{h^{2}}\left( u^{j+1}_{i + 1} - 2u_{i}^{j+1} + u^{j}_{i - 1} \right)  +  \dfrac{a^{2}}{h^{2}}\left( u^{j}_{i + 1} - 2u_{i}^{j} + u^{j}_{i - 1} \right)  \right)
$$