...
Разностная схема устойчива, если $\exists \{ C_{i} \}_{j = 1}^{2} > 0, \text{независящие от } h, j :\ \forall h, \forall j \quad \left\|\phi^j\right\|_{\Phi_h} \le C_{1} \left\| f^j \right\|_{F_h} + C_{2} \left\|g\right\|_{G_h}$

## 2.  Доказательство неравенства $\left\|(E + \sigma A)^{-1}\right\| \le 1$.

Пусть $A \ge 0$ (неотрицательно определённая), $\sigma \ge 0$.

Так как норма подчинённая, то $\left\|(E + \sigma A)^{-1}\right\| = \max\limits_\phi \dfrac{((E + \sigma A)^{-1} \phi, (E + \sigma A)^{-1} \phi)}{(\phi, \phi)}$

Пусть $\psi = (E + \sigma A)^{-1} \phi \implies \phi = (E + \sigma A)\psi$

$\left\|(E + \sigma A)^{-1}\right\| = \max\limits_{\psi} \dfrac{(\psi, \psi)}{((E + \sigma A)\psi, (E + \sigma A)\psi)} = \max\limits_{\psi} \dfrac{(\psi, \psi)}{(\psi, \psi) + 2\sigma(A\psi, \psi) + \sigma^2(A\psi, A\psi)} \le 1$
