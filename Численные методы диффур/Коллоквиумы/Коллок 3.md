...
Так как норма подчинённая, то $\left\|(E + \sigma A)^{-1}\right\| = \underset{ \phi }{ \sup } \dfrac{\left\| \left(E + \sigma A\right)^{-1} \phi \right\|}{\left\| \phi \right\|} = \max\limits_\phi \sqrt{ \dfrac{\left[(E + \sigma A)^{-1} \phi, (E + \sigma A)^{-1} \phi\right]}{(\phi, \phi)} }$
