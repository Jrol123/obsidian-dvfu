...
2. $\exists \alpha = \alpha \left( \delta \right) \quad \forall \epsilon > 0 \quad \exists \delta \left( \epsilon \right): \rho _{F}\left( \tilde{f}_{\delta}, \overline f \right) \leq \delta \left( \epsilon \right) \implies \rho _{Y}\left( \overline{ y }, \tilde{y}_{\alpha} \right) \leq \epsilon$
