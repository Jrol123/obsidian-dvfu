...
$\dfrac{u_i^{j+1} - u_i^j}{\tau} = a\dfrac{u_{i+1}^j -2u_i^j + u_{i-1}^j}{h^2}$ - разностная схема
...
Тогда порядок аппроксимации $O\left(\tau + h^2 + \dfrac{\tau}{h^2}\right)$.
...
Если порядок аппроксимации $O\left(\tau + h^2 + \dfrac{\tau}{h^2}\right)$, то она выполняется только если $\tau \to 0$ стремится быстрее, чем $h^2 \to 0$, следовательно, **условная аппроксимация**.
Если аппроксимация выполняется независимо от входных данных, то она **абсолютная**.

Устойчивость выполняется при $\lambda = \dfrac{a\tau}{h^2} \le \dfrac{1}{2}$:
$\underset{i}\max |u_i^{j+1}| \le |\lambda + (1-2\lambda) + \lambda|\cdot \underset{i}\max |u_i^j| = \max |u_i^j|$

В разностной схеме уравнения теплопроводности у правой части заменим $j$ на $j+1$, получим:

$\lambda u_{i-1}^{j+1} - (1+2\lambda)u_i^{j+1} + \lambda u_{i+1}^{j+1} = -u_i^j, \quad\quad i = \overline{1, N-1}$

$|1+2\lambda| - |\lambda| - |\lambda| = 1 \implies$ абсолютная устойчивость (и аппроксимация).
