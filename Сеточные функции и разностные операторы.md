...
## Сеточные нормы
$$\|y\|_c=\max\limits_{x\in \overline G}|y(x)|\quad \/y_h\/_с=\max\limits_{x\in\overline\omega_h}|y_n(x)|  \quad  ||y||_{L_{2}} = \left( \int \limits_{ 0 }^{ 1 } y^{2}\left( x \right) dx \right) ^{\frac 1 2} $$
$$
lr
$$