...
## Решение
Вспомним определение базы топологии:
Набор $\mathfrak S = \{ V_{i} | i \in I \}$
$\forall U \in T$ можно представить в виде $\underset {V \in \mathfrak S} \cup$
$(\implies):$
$\mathfrak S$ - база топологии $\implies \forall U \in T  \quad U = \underset{ i \in I }{ \cup } V_{i}  \quad  V_{i} \in \mathfrak S$
$\forall x \in U \implies \exists V_{i} \in \mathfrak S: x \in V_{i}$
$\implies \exists V_{i : \forall x in V_{i} \subset U$
$(\impliedby):$
