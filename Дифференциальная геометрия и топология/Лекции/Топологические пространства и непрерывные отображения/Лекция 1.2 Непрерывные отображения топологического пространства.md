...
**2.1.7 Утверждение**
	 ...
	 3. Если $\{ \overline{ a_{1} }, \ldots, \overline{ a }_{n} \}$ - базис $V$ и $\overline{ \gamma }(t) = \gamma'(t)\overline{ a_{1} } + \ldots + \gamma^{n}(t)\overline{ a }_{n}$, то $\gamma'(t_{0}) = \left( \gamma \right)$