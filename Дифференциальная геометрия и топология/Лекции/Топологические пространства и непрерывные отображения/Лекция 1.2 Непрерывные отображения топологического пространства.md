...
**2.1.7 Утверждение**
	 ...
	 1. 1
		 ...
		 2. Если $\overline{ u }, \overline{ v }: \left[ \alpha, \beta \right] \to V$ - вектор функции, и $\exists \overline{ u }'\left( t_{0} \right) , \overline{ v }'(t_{0})$, то $\left( \overline{ u }, \overline{ v } \right)'\left( t_{0} \right) = \left( \overline{ u }(t_{0}), \overline{ v }'(t_{0}) \right) + \left( u'(t_{0}), \overline{ v }() \right)$ 