...
**2.1.11 Свойства интеграла**
	...
	3. Если $\overline h(t) = h^{1}(t) \overline a_{1} + \ldots + \overline h^{n}\overline a_{n}$, где $\left\{ \overline a^{1},\ldots, \overline a^{n} \right\}$ - базис $V$, то $\int \limits_{ \alpha }^{ \beta }\overline h$