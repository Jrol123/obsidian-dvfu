 ...
 **2.1.6 Доказательство**
	 ...
	 $= \dfrac{\left| \overline{ a } + \overline{ \epsilon }(t) \right|^{2}\left| a \right|^{2} - \left( a, \overline{ a } + \overline{ \epsilon }(t) \right)^{2}}{\left| \overline{ a } + \overline{ \epsilon }(t) \right|^{2}} \implies$ переходя к пределу при $t \to t_{0}$ и учитывая, что $\lim \limits_{ t \to t_{0} }\overline{ \epsilon }(t) = 0$, то $\dfrac{\left| a \right|^{2}\left| a \right|^{2}}{}$
