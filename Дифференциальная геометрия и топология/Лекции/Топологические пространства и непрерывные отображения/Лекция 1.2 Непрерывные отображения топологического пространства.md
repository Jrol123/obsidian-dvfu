...
**Доказательство**
	$\left( \implies \right)$
		...
		$\overset{ \text{по определению} }{ \implies } \exists \delta > 0:\ B(x_{0}, \delta) \subset U$, то есть $f(B(x_{0}, \delta)) \subset V$, то есть $\forall x \in B(x_{0}, \delta),\ f(x) \in B\left( f(x_{0}) \right)$