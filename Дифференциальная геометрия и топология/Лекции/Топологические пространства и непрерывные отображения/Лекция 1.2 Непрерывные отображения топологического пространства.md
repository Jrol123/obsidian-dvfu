...
**2.1.11 Свойства интеграла**
	1. Если $\overline h$ неизвестная функция, то интеграл  $\int \limits_{ \alpha }^{ \beta } \overline h(t) dt$ однозначно определён
	2. $\int \limits_{ \alpha }^{ \beta }\left( \gamma \overline{ h }_{1}(t) + \delta \overline h_{2} \left( t \right) \right)dt = \gamma \int \limits_{  \alpha }^{ \beta }\overline h_{1}(t)dt + \delta \int \limits_{ \alpha }^{ \beta }\overline h$