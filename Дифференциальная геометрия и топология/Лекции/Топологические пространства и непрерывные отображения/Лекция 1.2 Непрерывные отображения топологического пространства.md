...
Тогда $\exists$ естественная параметризация кривой $\overline \gamma$, а именно $\tau(t) = \int \limits_{ \alpha }^{ t }\left| \gamma'(t) \right|dt$ является непрерывно дифференцируемой, строго возрастающей функцией $\tau : \left[ \alpha, \beta \right] \to \left[ 0, d \right]$, где $d = l\left( \gamma \right) = \int \limits_{  \alpha }^{ \beta }\left| \overline \gamma'(t) \right|dt$ так что у неё существует обратная $\phi: \left[ 0, d \right] \to \left[ \alpha, \beta \right]$, так что $\phi \left( \tau \left( t \right) \right) = t,\ \tau \left( \phi \left( x \right) \right) = x \quad \forall x \in \left[ 0, d \right]$
