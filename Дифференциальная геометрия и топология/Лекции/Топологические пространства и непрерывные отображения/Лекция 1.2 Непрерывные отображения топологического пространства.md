...
**2.1.7 Утверждение**
	 ...
	 1. Если $f: \left[ \alpha, \beta \right] \to R$ - вещественная функция, если $\exists f'(t_{0})$ и $\overline{ \gamma }'(t_{0})$, то $\left( fg \right)'\left( t_{0} \right) = f'\left( t_{0} \right) \cdot \overline{ \gamma }\left( t_{0} \right) + f(t_{0})$