...
**2.1.4 Определение**
	...
	Таким образом, $\overline{ a } = \overline{ \gamma }'(t_{0}) \iff \overline{ \gamma }(t) \overset{ def }{ = }\overline{ \gamma }(t_{0}) + \overline{ a }(t - t_{0}) + \overline{ \epsilon }(t)(t - t_{0})$, где $\lim \limits_{ t \to t_{0} }$