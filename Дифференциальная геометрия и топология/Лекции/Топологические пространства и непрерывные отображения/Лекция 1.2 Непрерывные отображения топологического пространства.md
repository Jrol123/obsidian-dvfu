...
**Доказательство**
Из определения предбпзы и свойств полного прообраза следует, что если $f:\ X \to Y$ - отображение топологического пространства и $PB$ - предбаза $Y$, то из условия, что $f^{-1}(W) \subset X$ - открыто $\forall W \in PB$, следует, что $f$ - непрерывно в каждой точке $X$
...
Покажем, что $PB = \{ \left( -\infty, \alpha \right), \left( \alpha, \infty \right) | \alpha \in \mathbb{R} \}$ - является предбазой $\mathbb{R}$. $B$ с. д., заметим, что $\left( \infty, \alpha \right) \cap \left( B, \infty \right) = \left( \beta, @ \right)$
